package com.raahmediq.common.config;

import com.raahmediq.ambulance.domain.Ambulance;
import com.raahmediq.ambulance.repository.AmbulanceRepository;
...
import com.raahmediq.bloodbank.domain.BloodBankSourceType;
...
import com.raahmediq.diagnostic.domain.DiagnosticProcedure;
import com.raahmediq.diagnostic.repository.DiagnosticProcedureRepository;
import com.raahmediq.hospital.domain.Department;
import com.raahmediq.hospital.domain.Hospital;
import com.raahmediq.hospital.repository.DepartmentRepository;
...
@Component
@ConditionalOnProperty(prefix = "raahmediq.demo-data", name = "enabled", havingValue = "true")
public class DemoDataLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        archiveLegacyDemoData();
        Hospital hospital = hospitals.findByCodeIgnoreCase("RMQ-DEMO").orElseGet(() -> {
            return hospitals.save(new Hospital("RMQ-DEMO", "RaahMediQ Health Demo Care Centre",
                    "18 Community Health Avenue", "New Delhi", "Delhi", "110001", "+911140404040",
                    "Asia/Kolkata"));
        });
        hospital.update("RMQ-DEMO", "RaahMediQ Health Demo Care Centre",
                "18 Community Health Avenue", "New Delhi", "Delhi", "110001", "+911140404040",
                "Asia/Kolkata", true);
        seedStaffAccounts();
    ...
    private void archiveLegacyDemoData() {
        hospitals.findByCodeIgnoreCase("RQV-DEMO").ifPresent(legacy -> {
            legacy.update("RMQ-LEGACY", "RaahMediQ Health Legacy Demo Centre", legacy.getAddressLine(),
                    legacy.getCity(), legacy.getState(), legacy.getPostalCode(), legacy.getContactNumber(),
                    legacy.getTimeZone(), false);
            for (Doctor doctor : doctors.findAllByHospitalIdAndActiveTrueOrderByNameAsc(legacy.getId())) {
                doctor.update(doctor.getDepartment(), doctor.getName(), doctor.getSpecialization(),
                        doctor.getRegistrationNumber(), doctor.getConsultationFee(),
                        doctor.getExpectedConsultationMinutes(), doctor.getDailyMaxCapacity(),
                        doctor.getBuilding(), doctor.getFloorLabel(), doctor.getRoomNumber(), false);
            }
        });
    }
    ...
    private void seedCareDirectory(Hospital hospital) {
        ...
        demoDoctor(hospital, medicine, "Dr. Ananya Mehta", "Internal Medicine", "RMQ-DMC-1042",
                "700.00", 15, 36, "Aarogya Block", "2nd Floor", "OPD 214", 9, 14);
        demoDoctor(hospital, cardiology, "Dr. Arjun Rao", "Cardiology", "RMQ-DMC-2087",
                "950.00", 20, 28, "Hriday Block", "3rd Floor", "OPD 307", 10, 15);
        demoDoctor(hospital, neurology, "Dr. Kavya Nair", "Neurology", "RMQ-DEMO-NEU-301",
                "1100.00", 20, 24, "Neuro Block", "2nd Floor", "OPD 228", 10, 14);
        demoDoctor(hospital, orthopaedics, "Dr. Rohan Singh", "Orthopaedic Surgery", "RMQ-DEMO-ORT-401",
                "850.00", 15, 32, "Mobility Block", "1st Floor", "OPD 118", 9, 14);
        demoDoctor(hospital, paediatrics, "Dr. Meera Iyer", "Paediatrics", "RMQ-DEMO-PED-501",
                "650.00", 15, 34, "Child Care Block", "1st Floor", "OPD 126", 9, 14);
        demoDoctor(hospital, gynaecology, "Dr. Sana Ahmed", "Obstetrics & Gynaecology", "RMQ-DEMO-OBG-601",
                "900.00", 20, 26, "Sakhi Block", "2nd Floor", "OPD 242", 10, 15);
        demoDoctor(hospital, ent, "Dr. Vivek Das", "ENT", "RMQ-DEMO-ENT-701",
                "700.00", 15, 30, "Speciality Block", "1st Floor", "OPD 132", 9, 13);
        demoDoctor(hospital, dermatology, "Dr. Isha Kapoor", "Dermatology", "RMQ-DEMO-DER-801",
                "750.00", 15, 30, "Speciality Block", "1st Floor", "OPD 136", 11, 16);
        demoDoctor(hospital, oncology, "Dr. Aditya Sen", "Medical Oncology", "RMQ-DEMO-ONC-901",
                "1200.00", 25, 20, "Cancer Care Block", "Ground Floor", "OPD 048", 10, 14);
        demoDoctor(hospital, ophthalmology, "Dr. Nidhi Verma", "Ophthalmology", "RMQ-DEMO-EYE-101",
    ...
    private void deactivateDuplicateDemoDoctors(Hospital hospital) {
        ...
        for (Doctor doctor : doctors.findAllByHospitalIdAndActiveTrueOrderByNameAsc(hospital.getId())) {
            ...
            Doctor duplicate = existing.getRegistrationNumber().startsWith("RMQ-") ? doctor : existing;
    ...
    private void seedAmbulances(Hospital hospital) {
        demoAmbulance(hospital, "DEMO-RMQ-AMB-01", "DEMO ALPHA", "Synthetic demo crew A",
                "Hospital demo bay");
        demoAmbulance(hospital, "DEMO-RMQ-AMB-02", "DEMO BRAVO", "Synthetic demo crew B",
                "Community route demo area");
        demoAmbulance(hospital, "DEMO-RMQ-AMB-03", "DEMO CHARLIE", "Synthetic demo crew C",
                "Hospital demo bay");
    }
    ...
    private void seedBloodSupport(Hospital hospital) {
        ...
        BloodBank hospitalBank = bloodBanks.findByHospitalIdAndCodeIgnoreCase(hospital.getId(), "RMQ-BB-MAIN")
                .orElseGet(() -> bloodBanks.save(new BloodBank(hospital, "RMQ-BB-MAIN",
                        "RaahMediQ Health Demo Blood Centre", "Synthetic development data — not a real facility", "+919999990100",
                        new BigDecimal("0.00"), 0, BloodBankSourceType.HOSPITAL_MANAGED)));
        BloodBank partnerBank = bloodBanks.findByHospitalIdAndCodeIgnoreCase(hospital.getId(), "RMQ-BB-PARTNER")
                .orElseGet(() -> bloodBanks.save(new BloodBank(hospital, "RMQ-BB-PARTNER",
                        "RaahMediQ Health Demo Partner Centre", "Synthetic development data — not a real facility", "+919999990101",
                        new BigDecimal("5.80"), 22, BloodBankSourceType.AUTHORIZED_PARTNER)));
        hospitalBank.updateDirectory("RaahMediQ Health Demo Blood Centre",
                "Synthetic development data — not a real facility", "+919999990100", new BigDecimal("0.00"), 0);
        partnerBank.updateDirectory("RaahMediQ Health Demo Partner Centre",
    ...
    private void seedNavigation(Hospital hospital) {
        ...
        checkpoint(hospital, node, "MAIN_ENTRANCE", "RMQ-DEMO-ENTRANCE", "Main entrance QR", "मुख्य प्रवेश QR");
        checkpoint(hospital, node, "MAIN_REGISTRATION", "RMQ-DEMO-REG", "Registration QR", "पंजीकरण QR");
        checkpoint(hospital, node, "LIFT_A_2", "RMQ-DEMO-LIFT-A2", "Lift A · 2nd floor QR", "लिफ्ट A · दूसरी मंज़िल QR");
        checkpoint(hospital, node, "LIFT_B_3", "RMQ-DEMO-LIFT-B3", "Lift B · 3rd floor QR", "लिफ्ट B · तीसरी मंज़िल QR");
        checkpoint(hospital, node, "LIFT_A_1", "RMQ-DEMO-LIFT-A1", "Lift A · 1st floor QR", "लिफ्ट A · पहली मंज़िल QR");
        checkpoint(hospital, node, "FIRST_FLOOR_RECEPTION", "RMQ-DEMO-OPD-1", "First-floor OPD help desk QR", "पहली मंज़िल ओपीडी सहायता डेस्क QR");
        checkpoint(hospital, node, "GENERAL_RECEPTION", "RMQ-DEMO-OPD-2", "Second-floor OPD reception QR", "दूसरी मंज़िल ओपीडी रिसेप्शन QR");
    }
