    ...
    public PolicyResult evaluate(String question) {
        ...
        if (CLINICAL_REQUESTS.stream().anyMatch(normalized::contains)) {
            return new PolicyResult(AiSafetyClass.CLINICAL_BOUNDARY,
                    "I can locate and summarize existing RaahMediQ Health records, but I cannot diagnose, prescribe, change a dose, or tell you to stop treatment. Please ask your qualified doctor or care team for that decision.");
        }
