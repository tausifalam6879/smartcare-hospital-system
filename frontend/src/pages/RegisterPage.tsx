  ...
  return (
    <AuthShell title="Create your patient account" subtitle="Only a mobile number, name, and secure password are required. Add other details when available.">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-bold text-ink-950">Date of birth<input type="date" max={localDateString()} value={form.dateOfBirth} onChange={(e) => update('dateOfBirth', e.target.value)} className={inputClass} /></label><label className="block text-sm font-bold text-ink-950">Language<select value={form.preferredLanguage} onChange={(e) => update('preferredLanguage', e.target.value)} className={inputClass}><option value="en">English</option><option value="hi">हिन्दी</option></select></label></div>
