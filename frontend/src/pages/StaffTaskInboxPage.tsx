...
export function StaffTaskInboxPage() {
  ...
  return <div className="min-h-screen bg-[#f5f8fc]"><section className="bg-ink-950 text-white"><div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8"><div><p className="text-xs font-black uppercase tracking-[.18em] text-blue-300">Role-aware operations</p><h1 className="mt-2 text-4xl font-black">My assigned work</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-blue-100/70">Requests become clear action cards with a next step. Overdue open work stays visible and the inbox refreshes every 15 seconds.</p></div><div className="flex flex-wrap items-end gap-3"><label className="text-xs font-bold text-blue-100">Hospital<select value={hospitalId} onChange={(event) => { const id = event.target.value; setHospitalId(id); void refresh(id) }} className="mt-1 block h-12 max-w-64 rounded-xl border border-white/15 bg-white px-3 text-sm font-bold text-ink-950">{hospitals.map((hospital) => <option key={hospital.id} value={hospital.id}>{hospital.name}</option>)}</select></label><Stat value={total} label="Open tasks" /><Stat value={urgent} label="Urgent" /><button onClick={() => void refresh(hospitalId)} className="grid size-16 place-items-center rounded-2xl border border-white/10 bg-white/10" aria-label="Refresh tasks"><RefreshCw className="size-5" /></button></div></div></section><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">{error && <p role="alert" className="mb-5 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-bold text-rose-800">{error}</p>}{total === 0 && <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center"><BadgeCheck className="mx-auto size-10 text-emerald-500" /><h2 className="mt-3 font-black text-ink-950">No open task right now</h2><p className="mt-1 text-sm text-slate-500">New assignments will appear automatically.</p></div>}
