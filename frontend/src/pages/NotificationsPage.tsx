import { Bell, BellRing, CheckCheck, Clock3, LoaderCircle, TicketCheck } from 'lucide-react'
...
export function NotificationsPage() {
  ...
  return (
    <div className="mx-auto min-h-[65vh] max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.2em] text-care-700">Care updates</p><h1 className="mt-2 text-4xl font-black tracking-tight text-ink-950">Notifications</h1><p className="mt-2 text-slate-600">Payment, check-in and live queue events from RaahMediQ Health.</p></div>{items.some((item) => !item.read) && <button onClick={() => void markAll()} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-extrabold text-slate-700"><CheckCheck className="size-4"/>Mark all read</button>}</div>
      {error && <p role="alert" className="mt-6 rounded-2xl bg-rose-50 p-4 text-sm font-semibold text-rose-800">{error}</p>}
      {loading ? <div className="mt-8 flex min-h-40 items-center justify-center gap-2 text-sm font-bold text-slate-500"><LoaderCircle className="size-5 animate-spin"/>Loading updates...</div> : items.length === 0 ? <div className="mt-8 rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center"><Bell className="mx-auto size-7 text-slate-300"/><h2 className="mt-4 font-black text-ink-950">No notifications yet</h2><p className="mt-2 text-sm text-slate-500">Booking and queue updates will appear here.</p></div> : <div className="mt-8 space-y-3">{items.map((item) => <article key={item.id} onClick={() => void markRead(item)} className={`cursor-pointer rounded-3xl border p-5 transition sm:p-6 ${item.read ? 'border-slate-200 bg-white' : 'border-care-200 bg-care-50/70 shadow-sm'}`}><div className="flex gap-4"><span className={`grid size-11 shrink-0 place-items-center rounded-2xl ${item.type === 'NOW_SERVING' ? 'bg-emerald-500 text-white' : 'bg-white text-care-700 shadow-sm'}`}>{item.type.includes('QUEUE') || item.type === 'NOW_SERVING' ? <BellRing className="size-5"/> : <TicketCheck className="size-5"/>}</span><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><h2 className="font-black text-ink-950">{item.title}</h2>{!item.read && <span className="rounded-full bg-care-600 px-2.5 py-1 text-[10px] font-black uppercase text-white">New</span>}</div><p className="mt-2 text-sm leading-6 text-slate-600">{item.message}</p><div className="mt-3 flex flex-wrap items-center gap-4 text-xs font-semibold text-slate-400"><span className="inline-flex items-center gap-1.5"><Clock3 className="size-3.5"/>{displayTime(item.createdAt)}</span>{item.appointmentId && <Link onClick={(event) => event.stopPropagation()} to={`/queue/${item.appointmentId}`} className="font-extrabold text-care-700 hover:underline">Open live visit</Link>}</div></div></div></article>)}</div>}
    </div>
  )
}
