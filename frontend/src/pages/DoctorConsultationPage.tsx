  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await finalizeClinicalVisit({
        ...
        followUpDate: followUpDate || undefined, medicationReminderEnabled: reminder,
      })
    ...
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      {loading ? <LoaderCircle className="mx-auto size-7 animate-spin text-care-600" /> : <>
        ...
        <form onSubmit={submit} className="grid gap-5 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <label className="text-sm font-bold">Eligible appointment<select required value={appointmentId} onChange={(event) => setAppointmentId(event.target.value)} className={`mt-2 w-full ${inputClass}`}><option value="">Choose appointment</option>{eligible.map((item) => <option key={item.id} value={item.id}>{item.serviceDate} · {item.departmentName} · OPD {item.queuePosition ?? '—'} · {item.status}</option>)}</select></label>
          {eligible.length === 0 && <p className="rounded-xl bg-amber-50 p-3 text-sm font-semibold text-amber-900">Call a checked-in patient to begin the consultation.</p>}
          ...
          <div className="grid gap-4 rounded-2xl border border-violet-200 bg-violet-50 p-5 md:grid-cols-2"><label className="text-sm font-bold text-violet-950"><CalendarCheck2 className="mr-2 inline size-5" />Follow-up date<input type="date" min={new Date().toISOString().slice(0, 10)} value={followUpDate} onChange={(e) => setFollowUpDate(e.target.value)} className={`mt-2 w-full ${inputClass}`} /></label><label className="flex items-center gap-3 self-end rounded-xl bg-white p-4 text-sm font-bold text-violet-950"><input type="checkbox" checked={reminder} onChange={(e) => setReminder(e.target.checked)} disabled={!followUpDate} className="size-5" />Enable medication reminder</label></div>
